...
class BaseAddon(object):
    ...
    def normalize_addon_connect_config(self, config):
        if 'pluginModules' in config and 'webhooks' in config['pluginModules']:
            webhooks = []
            for wh in config['pluginModules']['webhooks']:
                if not wh.get('url', None):
                    wh['url'] = reverse('atlassian-connect-django-webhook', kwargs={
                        'webhook_name': wh.get('webhookName', None) or wh['event']
                    })
                    if 'webhookName' in wh:
                        del wh['webhookName']
                webhooks.append(wh)
            config['pluginModules']['webhooks'] = webhooks
            return config
        return config
